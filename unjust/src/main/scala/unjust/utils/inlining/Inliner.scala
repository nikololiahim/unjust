...
import unjust.ObjectName
import unjust.utils.Abstract.modifyExpr
...
object Inliner {
  ...
  type CompleteObjectTree =
    ObjectTree[ObjectInfo[ParentInfo[MethodInfo, ObjectInfo], MethodInfo]]

  type ObjectTreeForInlining =
    ObjectTree[ObjectInfo[ParentInfoForInlining[MethodInfo], MethodInfo]]
  ...
  def createObjectTree(
    ...
    setLocators(prog).map(
      _.bnds
        .flatMap(bnd =>
          parseObject(bnd, 0, List()).map(obj => (obj.info.name, obj))
        )
        .toMap
    )
  }
  ...
  def zipWithInlinedMethod[P <: GenericParentInfo](
    ...
    obj.traverseMethods[P, MethodInfo, EitherNel[
      String,
      *
    ], MethodInfoAfterInlining, ObjectInfo]((name, curMethod, curObj) =>
  ...
  def resolveParents(
    ...
    ): EitherNel[String, Option[ParentInfo[MethodInfo, ObjectInfo]]] = {
      info match {
        case Some(info @ ParentName(ObjectNameWithLocator(locator, name))) =>
          ...
          val pathFromCtxToObj =
            names
              .map(name =>
                optionals.mapValueAtKey[EONamedBnd, PartialObjectTree](
    ...
    def recurse(
      cur: PartialObjectTree
    )(ctxs: Nel[PathToContext]): EitherNel[String, CompleteObjectTree] = {
          ...
          ObjectTree(
            ObjectInfoForAnalysis(
              ...
              parentInfo = cur
                ...
                .parentInfo
                .map(
                  _.asInstanceOf[ParentInfo[MethodInfo, ObjectInfoForAnalysis]]
                ),
